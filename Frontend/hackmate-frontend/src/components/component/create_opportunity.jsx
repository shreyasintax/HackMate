...
export function Create_opportunity() {
  ...
  const [formData, setFormData] = useState({
    ...
    contactDetails: '',
    rounds: [
      { description: '', deadline: '', resultDate: '' } // Initial round
    ],
    eligibility: {
      yearOfGraduation: '',
      age: '',
  ...
  const addRound = () => {
    if (formData.rounds.length < 4) {
      setFormData({
        ...formData,
        rounds: [...formData.rounds, { description: '', deadline: '', resultDate: '' }]
      });
    }
  };
  ...
  const handleChange = (e) => {
    ...
    // Automatically convert numeric inputs to numbers
    if (id.includes('yearOfGraduation') || id.includes('age') || id.includes('teamMembersNumber')) {
      newValue = Number(value); 
    } else if (type === 'checkbox') {
      newValue = checked; // Use the checked value for checkboxes
    }
  ...
  const handleImageChange = (event) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setImage(reader.resultDate);
        // Append image data to formData
        setFormData({ ...formData, image: reader.resultDate });
      };
                  ...
                  <div key={index}>
                    ...
                    <div className="flex gap-4">
                      ...
                      <Input
                        ...
                        onChange={(e) => handleRoundChange(index, 'resultDate', e.target.value)}
                        placeholder="Result"
                      />
                    </div>
